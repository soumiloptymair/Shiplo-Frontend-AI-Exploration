...
| Split Shipment modal                  | `pages/shipments/split-shipment-modal`                               | Designed in-house (no Figma frame). 760-wide modal with two columns (Shipment A / B). Each item starts on A; chevron buttons move items between columns. Footer Cancel + teal Confirm; Confirm disabled until both sides have ≥1 item. On confirm, fires the global toast. |
