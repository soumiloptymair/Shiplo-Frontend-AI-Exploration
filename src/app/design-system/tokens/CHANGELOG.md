...
- `pages/shipments/merge-shipment-modal` — 1324×812 dialog matching the split modal frame. Header reads `Merge Shipments — {primary.shipmentId}` with a subheader summarizing `{N of M} orders going to ZIP {zip} · Projected savings ${X.XX}`. Body: **left summary pane** (destination, projected savings, kept-warehouse radio group derived from the union of selected sources); **right scrollable column** containing one card per source (with include/exclude checkbox — primary checked + locked; uncheck disabled at minimum 2) followed by a teal "Merged Shipment Preview" card showing combined product list, total qty/value, kept warehouse, and projected savings. Confirm emits `{ sourceIds, keptWarehouse }` and is disabled when either constraint is unmet. `projectedSavings` prefers the seeded `mergeRecommendation.savings`, falling back to `extraPeers × $3.20`.
